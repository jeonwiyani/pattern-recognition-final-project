...
import trainingMultinomialLemmatization as trL
...
def multinomial(listTraining, listTesting):
    trainingReturnValue = trL.training(listTraining)
    ...
    for indexKomentar in range(len(listTesting)):
        ...
        if indexKomentar % 2 == 0:
            positif = True
        returnvalue = ts.testing(listTesting[indexKomentar], myConProb, termsMain, positif, inputKomentar)
...
inputDocument = pd.read_excel(
    r'D:/Tania/UB/Semester 5/Tania/PENGPOL/Project Akhir/Final Project/dataset.xlsx')
